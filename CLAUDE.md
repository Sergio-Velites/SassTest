...
**Fase actual: BACKLOG MVP COMPLETO (Ciclos 1–9).** El MVP funciona end-to-end en local: registro→organización→catálogo→instalación→ejecución con IA mock y aprobaciones→historial/logs, desde la UI y por API, con 100 ejecuciones concurrentes verificadas. El usuario ha decidido (2026-07-18) continuar con TODO en este orden: **Ciclo 10** despliegue GCP (código/IaC completos; el usuario crea los proyectos), **Ciclo 11** conectores reales (Slack, Google, IMAP/SMTP, Holded — detrás de config, mocks siguen de default), **Ciclo 12** editor visual React Flow, **Ciclo 13** billing Stripe en test mode. IA se queda en mock por ahora. Detalle en `docs/product/BACKLOG.md`. — monorepo, docs,
